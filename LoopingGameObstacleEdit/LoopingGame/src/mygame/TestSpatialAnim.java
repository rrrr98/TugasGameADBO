...
public class TestSpatialAnim extends SimpleApplication implements AnimEventListener {
    ...
    public void simpleInitApp() {
        ...
        obsMat.setColor("Color", ColorRGBA.Black);
        Box box = new Box(1f, 1f, 1f);
        Box obstacle = new Box(0.3f, 0.2f, 5f / 3f);
        ...
        for (int i = 0; i <= 10; i++) {
            ...
            //obsG.setLocalTranslation(new Vector3f(0,0,1.8f));
            //obsG.setLocalScale(now, now, now/3f);
            geom.setMaterial(m);
            leftG.setMaterial(m);
            rightG.setMaterial(m);
            obsG.setMaterial(obsMat);
            ...
            obs.setLocalTranslation(2 * i, 1.5f, factor);
    ...
    public void simpleUpdate(float tpf) {
        ...
        int x = r.nextInt(3);
        float factor = 1.8f;
//        Box obstacle = new Box(0.3f, 0.2f, 5f/4f);
//        Material obsMat= new Material(assetManager, "Common/MatDefs/Misc/Unshaded.j3md");
//        obsMat.setColor("Color", ColorRGBA.Black);
//        Geometry obsG = new Geometry("obstacle", obstacle);
//        obsG.setMaterial(obsMat);
//        obsG.setLocalTranslation(new Vector3f(0,0,factor));
//        rootNode.attachChild(obsG);
        //System.out.println(modelCharacter.getLocalTranslation().getY());
        if (IS_ALIVE) {
            if(mark)randomNumber += tpf;
            if(mark==false)randomNumber -=tpf;
                for (int i = 0; i < listOfObstacle.size(); i++) {
                        listOfObstacle.get(i).setLocalTranslation(0f, 0f, randomNumber);
                        System.out.println(randomNumber);
                }
                if(randomNumber>1.8)mark=false;
                if(randomNumber<-1.8) mark=true;
//            }
